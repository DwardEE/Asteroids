let gameStart = false; // main menu boolean
let pause = false;
const FPS = 30; // frames per second
const GAME_LIVES = 3; // starting number of lives
const SHIP_SIZE = 30; // ship height in pixels
const SHIP_INV_DUR = 3; // duration of ship's invicibility in seconds
...
const TEXT_SIZE = 40; // font size
const SAVE_KEY_SCORE = "highscore"; // save key score
const SOUND_ON = true; //sound switch
const MUSIC_ON = true; //music switch
...
function destroyAsteroid(index) {
    ...
    // caclulate the ratio of remaining asteroids to determin music tempo
    roidsLeft--;
...
function drawShip(x, y, a, colour = "white") {
    ...
    ctx.lineTo( // Read right of the Ship
        x - ship.r * (2 / 3 * Math.cos(a) - Math.sin(a)),
        y + ship.r * (2 / 3 * Math.sin(a) + Math.cos(a))
    );
    ctx.closePath();
...
function keyUp(/** @type {KeyboardEvent}**/ ev) {
    ...
    switch(ev.keyCode) {
        ...
        case 38: // stop thrust
            ...
            break;
        case 39: // stop roatate ship right
        ship.rot = 0;
            break;
    }
}
...
function update() {
    ...
    if (!gameStart) {
        ...
        ctx.fillText("ASTEROIDS", canv.width / 2, canv.height * 0.25);
        ...
        ctx.fillStyle =  "white";
        ctx.font = "small-caps " + TEXT_SIZE + "px dejavu sans mono";
        ctx.fillText("< Press Enter to Start >", canv.width / 2, canv.height * 0.50);
    } else if (!pause) {
        ...
        for (var i = roids.length - 1; i >= 0; i--) {
            ...
            // grab the the asteroid properties
            ax = roids[i].x;
            ay = roids[i].y;
            ar = roids[i].r;
        ...
        if (!exploding) {
            ...
            //reset the ship after the explosion is finished
